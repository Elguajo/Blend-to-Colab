...
3. Выполняйте главы сверху вниз: «Перед стартом», «Настройки», «Blender и GPU», «Загрузка проекта», «Preflight», «Рендер и resume», «Результаты».
...
В главах «Настройки — основные» и сворачиваемом «Настройки — Advanced» можно изменить следующие параметры:
...
### Opt-in кэш Blender в Google Drive

При `ENABLE_DRIVE_BLENDER_CACHE = True` notebook использует только выделенную папку
`/content/drive/MyDrive/Blend-to-Colab/blender-cache` для архива точной версии
Blender. На cache miss или при несовпадении checksum он скачивает архив только с
`download.blender.org`, сверяет SHA-256 из официального manifest и лишь затем
публикует его в кэш временным файлом с replace. Перед cache hit официальный
checksum запрашивается заново. Архив копируется в `/content`, где Blender
распаковывается и запускается; запуск с mounted Drive запрещён архитектурой
notebook.

Кэш не содержит `.blend`, ZIP-проектов, job folders, результатов или credentials.
12 сентября 2026 года в чистых runtime Colab подтверждены cache miss, cache hit
и восстановление 38-байтного повреждённого архива с повторной проверкой
официального SHA-256. Минимальные наблюдаемые результаты приведены в
[`docs/COLAB_DRIVE_CACHE_SMOKE_2026-09-12.md`](docs/COLAB_DRIVE_CACHE_SMOKE_2026-09-12.md).
Кэш остаётся opt-in.
