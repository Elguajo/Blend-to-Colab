...
- [ ] Зафиксировать JSON Schemas для `job.json`, `status.json` и `result_manifest.json`.
- [ ] Ввести уникальный job id и версию протокола.
- [ ] Реализовать state machine и монотонную ревизию статуса.
- [ ] Планировать chunks и только отсутствующие кадры.
- [ ] Записывать streaming log и итоговый summary.
- [ ] Публиковать manifest и результаты атомарно.
- [ ] Проверять checksum каждого подтверждённого результата.
- [ ] Добавить unit/integration tests переходов состояния, resume и повреждённых результатов.
- [ ] Имитировать обрыв runtime и повторный запуск в реальном Colab.
